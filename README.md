...
- 复习、主题、题库和统计四个入口
- 最近一次作答仍错误的错题索引，并可定位回原题
- 从笔记 frontmatter 和正文标签建立的层级主题索引
- 测试题总数、题目总数、已测试数量和完成次数
- 全局及 L1–L4 首次正确率
- 单选、多选、判断和填空题分布
- 最近 8 个自然周的首次回答量和正确率趋势
- 按标题、Quiz ID、文件路径或主题搜索
- 模式、主题和学习状态筛选，以及最近学习、待复习、正确率和标题排序
- 当前进度、最近活动和一键打开原笔记
- 无效 Quiz block 与重复 Quiz ID 提示

面板统计只关联知识库中仍然存在的题目。“首次正确率”按每次测验会话中每道题的第一次回答计算，重新作答不会覆盖薄弱点数据。错题集只保留跨会话最新一次作答仍错误的问题；后续答对后会自动移除。
